...
function DashboardClient() {
  ...
  return (
    <main className="relative min-h-screen px-4 sm:px-6 lg:px-8 pt-6 pb-24 overflow-hidden bg-gradient-to-br from-pink-50 via-white to-purple-50">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -top-40 -left-40 h-[420px] w-[420px] sm:h-[520px] sm:w-[520px] rounded-full bg-pink-400/25 blur-3xl" />
        <div className="absolute top-1/3 -right-40 h-[420px] w-[420px] sm:h-[520px] sm:w-[520px] rounded-full bg-purple-400/25 blur-3xl" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(236,72,153,0.10),transparent_60%)]" />
      </div>

      <div className="relative max-w-6xl mx-auto space-y-10 sm:space-y-12 animate-fade-in">
        <section className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="text-center lg:text-left">
            <p className="text-xs uppercase tracking-wider text-zinc-400">
              Welcome back
            </p>
            <h1 className="mt-1 text-2xl sm:text-3xl font-bold bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent">
              {name?.split(" ")[0]}
            </h1>
          </div>

          <div className="lg:absolute lg:left-1/2 lg:-translate-x-1/2 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 sm:gap-6">
            {tools.map((tool) => (
              <Link
                key={tool.title}
                href={tool.href}
                className="group relative rounded-3xl p-5 sm:p-6 bg-gradient-to-br from-pink-500/10 via-white/10 to-purple-500/10 backdrop-blur-2xl border border-purple-500/30 ring-1 ring-purple-500/20 shadow-sm transition-all duration-300 ease-out hover:-translate-y-1 hover:border-transparent hover:ring-2 hover:ring-pink-500/50 hover:shadow-[0_0_45px_rgba(168,85,247,0.35)]"
              >
                ...
                <div className="relative">
                  ...
                  <h3 className="mt-4 text-base sm:text-lg font-semibold text-zinc-900 group-hover:text-purple-700 transition">
                    {tool.title}
                  </h3>
        ...
        <section className="rounded-2xl p-4 sm:p-5 bg-red-50 border border-red-300">
...
function Stat({ label, value }: { label: string; value: any }) {
  return (
    <div className="rounded-xl px-3 sm:px-4 py-2 text-xs bg-white/40 backdrop-blur-xl border border-purple-500/30 shadow-sm">
